...
<body>
	...
	<script type="text/javascript">
		function init(){
			...
			//查询制度详情
			var dataURL = "{contextPath}/com_regulation/queryRegulationDetail.action";
			var jPost= {};
				...
				if (!json || json.errorCode != "0") {
					...
					if(data.auditFileList.length > 0){
						for(var i=0;i < data.auditFileList.length;i++){
							var auditFile = "";
							auditFile = auditFile + "<a onclick='showFileView('"+data.auditFileList[i].FILE_ID+"')' id='"+data.auditFileList[i].FILE_ID+"'>"+data.auditFileList[i].FILE_NAME+"</a>";
