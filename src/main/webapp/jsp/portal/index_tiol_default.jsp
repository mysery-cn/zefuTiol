...
<script type="text/javascript">
	...
	//首页地址
	var CST_URL_INDEX = "/jsp/statistics/company/index.jsp";
	//页面打开后跳转到Hash指定的地址
	...
	function showDesktop(){
		...
		showWindow(CST_URL_INDEX,'frameMain');
		jt.setHash('Tree','');
	}
...
//		}
	...
	function loadTopMenu(){
		...
		loadLeftMenu(jt.getDefVal(jt.getHash('TopMenu'),0), jt.getDefVal(jt.getHash('LeftMenu'),0));
		setIdxNavClass();
	}
	...
	function loadLeftMenu(idx,idxSub){
		...
		if (menuIdx_Top==idx) {  //已经加载了TreeView ///TODO 处理  选择节点;
			//jt._('#divOutlook').selectItem(idxSub);
			return;
		}
		menuIdx_Top=idx;
        ...
        jt._('#idx_SplitBar').style.display = "none"
		setIdxNavClass();
		bodyResize();
...
//		}
	...
	<style type="text/css">
	.top_search{width:240px; height:28px;background: #fff;margin-right: 40px;}
.top_search input.search_txt{width:215px; height:28px; right:25px;border:0; background:none;background-color: #fff;font-size: 14px;line-height: 32px;padding-left: 4px;float: left;}
.search_btn{width:20px; height:22px;cursor:pointer; border:0; background-image: url('<%=SYSURL_static%>/images/tiol/btn_search.png');float: left;background-color: #fff;margin-top: 3px;}
</style>
<body class="BodyIndex">
	<table width="100%" id="idxTableMain" style="height:100%;table-layout:fixed; " border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="64" id="idxTop" class="idx_top">
				<div class="idx_top_In" style="position:relative; width:100%; height:64px;background-color: #417ac0;"><!--用于做双背景-->
					<table class="idx_Links" width="100%" style="display:none;" border="0" cellspacing="0" cellpadding="0" hidden="hidden" >
						<tr>
					...
					<a href="javascript:void(0);" onclick="showDesktop();return false;">
						<img id="imgLogo" src="<%=SYSURL_static%>/images/tiol/logo.png" style="position:absolute; top:15px; left:20px;" onload="fixPNG(this,500,55)"/>
					</a>
					
					<table border="0" cellspacing="0" cellpadding="3" style="position:absolute; top:10px; right:12px;">
						<tr>
						    <td>
						        <div class="top_search" style="display: none;">
						            <input class="search_txt" type="text" placeholder="请输入搜索内容">
         			                <input class="search_btn" type="button">
         			            </div>
						    </td>
							<td><img id="imgUserPhoto" src="<%=SYSURL_static%>/images/tiol/head_bg.png" style="cursor:pointer;" onclick="showUserInfo();"></td>
							<td align="right" style="padding-right:5px;line-height:20px;">
								<span id="spanUserName"><%=loginUser==null?"":loginUser.getUserName() %></span><br>
								<span id="spanUserDept"><%=loginUser==null?"":loginUser.getDeptName() %></span>
							</td>
							<td >
								<a style="color:#fff;">|&nbsp;&nbsp;</a>
							</td>
							<td><a style="color:#fff;" href="javascript:void(0);" onclick="logout();return false;">安全退出</a></td>
						</tr>
					</table>
					<div id="divSC_ShowHide" class="divSC_ShowHide_Show" style="position:absolute; top:77px; right:18px;" onclick="showHideSC()"></div>
				</div>
			</td>
		</tr>
		<tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="height:100%;table-layout:fixed;">
  <tr height="100%" valign="top">
		<td class="idx_Nav idx_Nav_white" id="idx_Nav" width="195" style="border-bottom:#d5d5d5 1px solid;border-left:#d5d5d5 1px solid;" >
		...
		<td id="idx_SplitBar" class="SplitBar SplitBar_toRight" minLeft="100" afterClick="this.hideLeft();" afterresize="setIdxNavClass();" width="6" style1="position:relative;"><span></span></td>
    <td id="idx_Main" class="idx_Main">
			<iframe name="frameMain" id="frameMain" src="" frameborder="0" style="width:100%;height:100%;background-color:white;" onload="setFrameMainClass()"></iframe>
		</td>
