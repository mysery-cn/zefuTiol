...
<script type="text/javascript">
	 ...
	 * 检查密码是否需要更新
     */
	function checkPwdStatus(){
        getJSON("{contextPath}/ums/user/checkUpdatePassword.action", function(j,o){
            if(j.data[0] != "1"){
                updatePwd();
            }
        });
	 ...
	 * 检查用户是否在其他设备登录
     */
	function checkUserLoginAnotherDevice(){
        getJSON("{contextPath}/anonymous/checkUserLogInAnotherDevice.action", function(json,o){
            if(json.errorCode != 0){
                jt.Msg.alert("用户已在其他设备登录");
				logout();
            }
			else{
				setTimeout(function(){
			...
			<td height="64" id="idxTop" class="idx_top">
				<div class="idx_top_In" style="position:relative; width:100%; height:64px;background-color: #417ac0;"><!--用于做双背景-->
					...
					<!-- <a href="javascript:void(0);" onclick="showDesktop();return false;"> -->
						<img id="imgLogo" src="<%=SYSURL_static%>/images/tiol/com_logo.png" style="position:absolute; top:15px; left:20px;" onload="fixPNG(this,500,55)"/>
					<!-- </a> -->
